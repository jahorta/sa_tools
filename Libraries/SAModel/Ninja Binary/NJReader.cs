				...
				switch (chunk.Type)
				{
					...
					case NinjaBinaryChunkType.SimpleShapeMotion:
						try
						{
							...
							Motions.Add(new NJS_MOTION(chunk.Data, 0, (uint)chunk.ImageBase, Models.Count > 0 ? Models[modelCount - 1].CountAnimated() : -1, shapeLabels, numverts: Models[modelCount - 1].GetVertexCounts()));
						}
						catch (Exception ex)
						{
